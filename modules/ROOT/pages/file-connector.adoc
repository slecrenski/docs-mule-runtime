...
*Reference*: xref:file-transport-reference.adoc[File Transport Reference]
...
This dependency on the file system and the OS imply some caveats using the file transport in a HA environment with a remote shared folder. We have verified that in the case of Windows shared folders there is a latency in the update of file metadata so a node in a cluster may eventually take an already deleted file as still existing. This may result in reprocessing of a file if the autodelete flag is set. We have added some system properties to ameliorate this problem:

`-M-Dmule.transport.file.not.recently.files.processing.period=30000` you can use this system property so that after a file with a certain name it is not reprocessed by any node in 30000 miliseconds.

`-M-Dmule.transport.file.singlepollinstance`: you can use this flag so that only the primary node polls the folder for files to processing.
...
|*Enable default events tracking* |Enable default  xref:business-events.adoc[business event]  tracking for this endpoint.
|===
...
Review xref:dataweave-flat-file-schemas.adoc[DataWeave FlatFile Schemas] doc for information on how to define the file format.
